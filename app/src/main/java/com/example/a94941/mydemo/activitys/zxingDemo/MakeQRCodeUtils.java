...
/**
 * Created by ws on 2017/6/30 0030.
 * 生成二维码的工具类
 */
public class MakeQRCodeUtils {
    ...
    public static Bitmap makeQRImage(Bitmap logoBmp, String content, int QR_WIDTH, int QR_HEIGHT) throws WriterException {
        try {
            ...
            for (int y = 0; y < QR_HEIGHT; y++) {
                ...
                for (int x = 0; x < QR_WIDTH; x++) {
                    if (bitMatrix.get(x, y)) {
                        ...
                        if (x < QR_WIDTH / 2 && y < QR_HEIGHT / 2) {
                            pixels[y * QR_WIDTH + x] = 0xFF47B135;// 绿色
                            //Integer.toHexString(new Random().nextInt());
                        } else if (x < QR_WIDTH / 2 && y > QR_HEIGHT / 2) {
                            pixels[y * QR_WIDTH + x] = 0xFFE70012;// 红色
                        } else if (x > QR_WIDTH / 2 && y > QR_HEIGHT / 2) {
                            pixels[y * QR_WIDTH + x] = 0xFFF29900;// 橘黄色
                        } else {
                            pixels[y * QR_WIDTH + x] = 0xFFE5007E;// 粉红
                        }
            ...
            // 图片绘制在二维码中央，合成二维码图片
            // logo大小为二维码整体大小的1/4
            float scaleFactor = QR_WIDTH * 2.0f / 9 / logoWidth;
            try {
    ...
    public static Bitmap addBackground(Bitmap foreground, Bitmap background){
        ...
        canvas.drawBitmap(foreground, (bgWidth - fgWidth) / 2, (bgHeight - fgHeight) * 2 / 5, null);
        canvas.save(Canvas.ALL_SAVE_FLAG);
        canvas.restore();
